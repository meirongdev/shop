...
| `buyer-portal` | 买家 SSR 门户 | Kotlin、Thymeleaf | API client、Controller、模板与条件渲染 |
| `seller-portal` | 卖家 SSR 门户 | Kotlin、Thymeleaf | API client、Controller、模板与表单流程 |
...
- 统一注入 `X-Request-Id`、`X-Player-Id`、`X-User-Id`、`X-Internal-Token` 等可信头。
...
Portal 的角色不是再发明一套业务服务，而是：

- 复用 Gateway / BFF 的 northbound API。
- 通过 Kotlin + Thymeleaf 快速表达页面结构、表单和 SSR 流程。
