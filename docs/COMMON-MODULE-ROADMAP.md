...
| InternalAccessFilter | 4 | servlet-api | 域服务 | **移除**（由 K8s NetworkPolicy 替代） |
...
### 4.3 InternalAccessFilter 移除计划

当前的 `InternalAccessFilter` 是共享静态密钥校验，存在以下问题：
...
2. 验证 BFF → 域服务调用正常
3. 将 `shop.security.internal.enabled` 设为 `false`
4. 移除 InternalAccessFilter 相关代码和所有服务的 `SHOP_INTERNAL_TOKEN` 配置
...
InternalAccessFilter ──→ 移除（NetworkPolicy 替代）
...
| `SECURITY-BASELINE-2026.md` | InternalAccessFilter 移除后需同步更新东西向安全章节 |
