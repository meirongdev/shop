...
public class BuyerRegisteredListener {
    ...
    public void onBuyerRegistered(String payload) {
        try {
            EventEnvelope<BuyerRegisteredEventData> event = objectMapper.readValue(
                    payload,
                    new TypeReference<>() {}
            );
            validateEvent(event);
            ...
            log.info("Received buyer.registered event: buyerId={}", data.buyerId());
            ...
            throw new NonRetryableKafkaConsumerException("Malformed notification buyer registered event", exception);
        } catch (IllegalArgumentException exception) {
            throw new NonRetryableKafkaConsumerException("Invalid notification buyer registered event", exception);
        } catch (DataAccessException exception) {
            throw new RetryableKafkaConsumerException("Temporary notification buyer registered processing failure", exception);
        }
    }

    @DltHandler
    public void handleDlt(String payload) {
        log.error("Notification buyer registered event sent to DLT: {}", payload);
    }

    private void validateEvent(EventEnvelope<BuyerRegisteredEventData> event) {
        if (event == null || event.data() == null) {
            throw new IllegalArgumentException("Notification buyer registered event data is required");
        }
