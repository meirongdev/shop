...
if [[ "${flow}" == "legacy" ]]; then
  ...
  bash "${repo_root}/platform/scripts/build-images.sh" --changed --legacy -j "${jobs}"
  bash "${repo_root}/platform/scripts/load-images-kind.sh" "${cluster_name}" --changed --kind-load
  bash "${repo_root}/platform/scripts/deploy-kind.sh" "${overlay}" --all --legacy
  phase_done
else
  phase_start "Maven build (clean package, -T 1C)"
  bash "${repo_root}/platform/scripts/build-images.sh" --changed --fast -j "${jobs}"
  phase_done
